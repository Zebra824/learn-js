...
function createButNode(txt, e, l) {
  let butNode = document.createElement("button");
  let btnNodeText = createTxtNode(txt);
  butNode.appendChild(btnNodeText);
  butNode.addEventListener(e, l);
...
function addTable() {
  ...
  for(let i = 0; i < 3; i++) {
    ...
    let col2 = createTDNode(createButNode("Edit text", "click", function(){edit(col1)}));
